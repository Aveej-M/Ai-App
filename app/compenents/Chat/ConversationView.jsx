...
import { conversationData } from "../../data/chat";

const ConversationView = ({ setOpenConversationView }) => {
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      setOpenConversationView(false);
    }, 400); // match animation duration
  };

  const filterConversationTitle = 
    conversationData.filter(item => item.label.toLowerCase().includes(searchInput.toLowerCase()));
    ...
    <div className="h-full w-full bg-black/30 absolute top-0 left-0 z-50 flex justify-center">
      ...
      >
        <div className="p-5 bg-gradient-to-r from-green-500 to-green-200 flex items-center gap-3">
          ...
          >
            <i className="fa-solid fa-xmark text-2xl pt-1"></i>
          </div>
        ...
        <div className="px-8">
            ...
            {openConversations && (
                <div>
                    {filterConversationTitle.map(({label, count}, index) => (
                        <div key={index} className="flex items-center gap-5">
                            {openBookMarks && (
                                <i className="fa-regular fa-bookmark text-green-500 cursor-pointer hover:bg-gray-200 p-2 px-2.5 rounded transition duration-200"></i>
                            )}
                            
                            <div className="justify-items py-2 text-gray-500 w-full">
                                <div className={`${openBookMarks ? "" : "cursor-pointer"}`}>{label}</div>
                                <div>{count}</div>
                            </div>
                        </div>
                    ))}
